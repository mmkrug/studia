...
public class ask_wykres extends JPanel{
    ...
    static double max_value = 50;
    static double min_value = -10;
    ...
    public void paintComponent(Graphics g){
        ...
        // Oś X
        {
            ...
            g.drawString("" + ilosc_probek, wymiar.width - 22, (int)(Math.abs(0-max_value)/wysokosc*wymiar.height) + 12);
...
//            if(bity[iteracja_bitow]==0){
//                suma += Math.abs(A_1*Math.sin(2*Math.PI*f_n*x));
//                wartosci_Y[i] = suma;
//            }else{
//                suma += Math.abs(A_2*Math.sin(2*Math.PI*f_n*x));
//                wartosci_Y[i] = suma;
//            }


            if(bity[iteracja_bitow]==0){
                suma1 += Math.abs(A_1*Math.sin(2*Math.PI*f_n*x));
                suma2 += Math.abs(A_2*Math.sin(2*Math.PI*f_n*x));
                wartosci_Y[i] = suma1 - suma2;
            }else{
                suma1 += Math.abs(A_1*Math.sin(2*Math.PI*f_n*x));
                suma2 += Math.abs(A_2*Math.sin(2*Math.PI*f_n*x));
                wartosci_Y[i] = suma2 - suma1;
            }
    ...
    public static void main(String[] args){
        ...
        ask_wykres graphics = new ask_wykres();
        f.add(graphics);
