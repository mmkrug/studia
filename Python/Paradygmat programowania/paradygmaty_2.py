...
def pomnoz(dane_wej,rozwiazanie,iloczyn):
    if(dane_wej[3]!=0):
        if( np.imag(rozwiazanie[0]) != 0 ):
            iloczyn.append( rozwiazanie[1]*rozwiazanie[2]*rozwiazanie[3]*rozwiazanie[4] )
            
    elif(np.real(rozwiazanie[0]) > 0 ):
        iloczyn.append( np.real( rozwiazanie[1])*np.real(rozwiazanie[2])  )
        
    elif(np.real(rozwiazanie[0]) < 0 ):
        iloczyn.append( rozwiazanie[1]*rozwiazanie[2] )
    return iloczyn


def podziel(dane_wej,rozwiazanie,iloraz):
    if( np.imag(rozwiazanie[0]) != 0 ):
        iloraz.append(rozwiazanie[1]/rozwiazanie[2]/rozwiazanie[3]/rozwiazanie[4])
        
    elif(np.real(rozwiazanie[0]) > 0 ):
        if(dane_wej[0] != 0 and dane_wej[3] == 0 and dane_wej[0] > 0):
            if(np.real(rozwiazanie[2] != 0)):
                iloraz.append(rozwiazanie[1]/rozwiazanie[2])
                
    elif(np.real(rozwiazanie[0]) < 0 ):
        iloraz.append((np.real(rozwiazanie[1])*np.real(rozwiazanie[2])+np.imag(rozwiazanie[1])*np.imag(rozwiazanie[2]))/(np.power(np.real(rozwiazanie[2]),2)+np.power(np.imag(rozwiazanie[2]),2)))
        iloraz.append((np.real(rozwiazanie[2])*np.imag(rozwiazanie[1])-np.real(rozwiazanie[1])*np.imag(rozwiazanie[2]))/(np.power(np.real(rozwiazanie[2]),2)+np.power(np.imag(rozwiazanie[2]),2)))
        
    return iloraz


# =============================================================================
# def przeciwny(dane_wej,rozwiazanie,przeciw):
#     if(dane_wej[0] != 0 and dane_wej[3] == 0 and rozwiazanie[0] == 0 and):
#         if( np.imag(rozwiazanie[0]) != 0 ):
#             iloraz.append(rozwiazanie[1]/rozwiazanie[2]/rozwiazanie[3]/rozwiazanie[4])
#         
#     elif(np.real(rozwiazanie[0]) > 0 ):
...
def wyswietlenie(rownanie,rozwiazanie,suma,roznica,iloczyn):
    ...
    if(rownanie[0]!=0 and rownanie[3]==0):       
        ...
        if(np.real(rozwiazanie[0]) > 0):
            ...
            print('Iloczyn: ',np.real(iloczyn[0]))
...
iloczyn = pomnoz(dane_wej,rozwiazanie,iloczyn)
iloraz = podziel(dane_wej,rozwiazanie,iloraz)
